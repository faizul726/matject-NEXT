...
if "%PROCESSOR_ARCHITECTURE%" equ "AMD64" (
    powershell -Command "Invoke-WebRequest https://github.com/jqlang/jq/releases/latest/download/jq-windows-amd64.exe -OutFile jq.exe"
) else (
    if "%PROCESSOR_ARCHITECTURE%" equ "x86" (
        powershell -Command "Invoke-WebRequest https://github.com/jqlang/jq/releases/latest/download/jq-windows-i386.exe -OutFile jq.exe"
    )
)

if exist "jq.exe" (echo !GRN![*] Downloaded!RST!) else (echo !RED![*] Download FAILED!RST! && echo Press any key to close... && pause > NUL && exit)
echo.
pause && goto:EOF
...
if not exist "jq.exe" (echo !RED![^^!] jq.exe not found.!RST! && echo. && echo Press any key to close... && pause > NUL && exit)
